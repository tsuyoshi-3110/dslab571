...
/** ▼ 価格を任意にするため、price/taxIncluded をオプショナルへ */
type ProdDoc = Omit<Product, "price" | "taxIncluded"> & {
  price?: number;
  taxIncluded?: boolean;
  base?: Base;
  t?: Tr[];
...
/** 表示テキスト多言語解決 */
function displayOf(
  p: { title?: string; body?: string; base?: Base; t?: Tr[] },
  lang: UILang
): Base {
  const fallback: Base = {
...
export default function ProductsClient() {
  ...
  const [price, setPrice] = useState<number | "">(""); // 任意価格（空OK）
    ...
    const unsub = onSnapshot(firstQuery, (snap) => {
      const rows: ProdDoc[] = snap.docs.map((d) => {
        ...
        return {
          ...
          title: data.title ?? "",
          body: data.body ?? "",
          price: typeof data.price === "number" ? data.price : undefined, // 任意
          mediaURL: data.mediaURL ?? data.imageURL ?? "",
          mediaType: (data.mediaType ?? "image") as MediaType,
          originalFileName: data.originalFileName,
          taxIncluded:
            typeof data.taxIncluded === "boolean" ? data.taxIncluded : undefined, // 任意
  ...
  const saveProduct = async () => {
    ...
    try {
      ...
      if (formMode === "edit" && editing) {
        await updateDoc(doc(productColRef, editing.id as string), payload);
      } else {
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    newList.forEach((item, index) => {
      batch.update(doc(productColRef, item.id as string), { order: index });
    });
    await batch.commit();
  };
  ...
  const handleGenerateWithKeywords = async () => {
    const keywords = [kw1.trim(), kw2.trim(), kw3.trim()].filter(Boolean);
    ...
    try {
      ...
      const res = await fetch("/api/generate-description", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title, price: priceValue, keywords }),
      });
    ...
    <main className="max-w-5xl mx-auto p-4 pt-10">
      ...
      {showGenModal && (
        <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/50 p-3">
          <div className="w-full max-w-md bg-white rounded-lg p-5 space-y-4">
            <h3 className="text-lg font-bold text-center">キーワードを指定してAI生成</h3>
            ...
            <div className="flex gap-2 justify-center pt-2">
              ...
              <button
                ...
                disabled={
                  aiLoading || [kw1.trim(), kw2.trim(), kw3.trim()].filter(Boolean).length < 1
                }
